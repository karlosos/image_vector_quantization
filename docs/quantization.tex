...
\begin{itemize}[label=]
    \item $C_S$ - codebook size - rozmiar książki kodowej w bitach
    \item $I_S$ - image size - rozmiar obrazu oryginalnego
    \item $Q_S$ - quantized size - rozmiar obrazu po kwantyzacji (wielkość słownika + kodowanie poszczególnych bloków)
\end{itemize}

W celu obliczenia procentowej ilości jaką stanowi rozdzielczość obrazu względem wielkości zdefiniowanego słownika należało skorzystać ze wzorów:

\begin{itemize}[label=]
    \item $C_S$ - skorzystano ze wzoru \ref{eq:lgb_dict_size} w tym dokumencie
    \item $Q_S$ - skorzystano ze wzoru \ref{eq:lgb_image_size} w tym dokumencie
\end{itemize}
